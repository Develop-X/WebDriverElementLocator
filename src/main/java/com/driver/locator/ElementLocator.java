...
public class ElementLocator {
    ...
    public ElementLocator() {
        ...
        ((DesiredCapabilities) caps).setCapability(PhantomJSDriverService.PHANTOMJS_EXECUTABLE_PATH_PROPERTY, "src/overview/phantomjs.exe");

        //        System.setProperty("webdriver.chrome.driver","src/overview/chromedriver.exe");
        rReader = new PropertyFileReader();
//		dDriver = new ChromeDriver();
        dDriver = new PhantomJSDriver(caps);
        dDriver.manage().timeouts().pageLoadTimeout(60, TimeUnit.SECONDS);
    }
